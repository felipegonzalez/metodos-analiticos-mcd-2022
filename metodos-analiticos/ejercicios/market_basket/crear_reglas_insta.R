...
#Se requeren un poco más de 6G de memoria para correr este script
...
trans <- read.transactions(ruta_trans, format = "single",
                  sep = ",", cols = c("order_id", "product_id"), header = TRUE)
trans
inspect(trans[1:2])


# nombres de productos
productos_tbl <- read_csv(here("datos", "instacart-kaggle/products.csv"),
                          col_types = "icii") |>
...
obtener_recoms <- function(canasta){
  n_canasta <- length(canasta)
  recoms <- subset(reglas, lhs %ain% canasta) |>
    subset(subset = size(lhs) <= n_canasta) |>
    DATAFRAME() |>
    group_by(RHS) |> top_n(10, wt = hyper_lift) |>
...
canasta <- c( "41844", "24852")
obtener_nombres(canasta)
recoms <- obtener_recoms(canasta)
recoms

canasta <- c( "16696", "10957")
obtener_nombres(canasta)
recoms <- obtener_recoms(canasta)
recoms
