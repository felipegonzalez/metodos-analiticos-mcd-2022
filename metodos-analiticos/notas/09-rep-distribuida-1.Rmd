...
entrena_bigramas <- map(texto_entrena, 
  ~tokenizers::tokenize_ngrams(.x, n = 2)[[1]]) |> 
  flatten() |> unlist()
entrena_trigramas
```

```{r}
log_p <- sapply(entrena_bigramas, function(x) feed_fow_bigrama(x, C, H, U))
sum(log_p)
```
...
model |> closest_to(vector, n = 8) |> filter(word != "tío")
```
